...
Parameter quad_img_left  :
  forall j k, inV3 ((adjM (A ^+ k) *m (A ^+ (S j))) *v v).
Parameter quad_img_right :
  forall j k, inV3 ((adjM (A ^+ (S k)) *m (A ^+ j)) *v v).

Lemma inV3_inner_jk (j k : nat) :
  inV3 (((alpha *: (adjM (A ^+ k) *m (A ^+ (S j))))
        +m ((Conj alpha) *: (adjM (A ^+ (S k)) *m (A ^+ j)))) *v v).
Proof.
  apply inV3_of_sum_matrices.
  - apply inV3_of_scaled_matrix, quad_img_left.
  - apply inV3_of_scaled_matrix, quad_img_right.
Qed.

Lemma inV3_inner_j (j : nat) :
  inV3 ((sum3M (fun k => (c j k) *:
                   ((alpha *: (adjM (A ^+ k) *m (A ^+ (S j))))
                  +m ((Conj alpha) *: (adjM (A ^+ (S k)) *m (A ^+ j))))) ) *v v).
Proof.
  apply inV3_sum3M_apply;
    (apply inV3_of_scaled_matrix; apply inV3_inner_jk).
Qed.
...
Proof.
  unfold Wop.
  apply inV3_sum3M_apply; apply inV3_inner_j.
Qed.
